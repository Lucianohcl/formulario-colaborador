...
# ==============================================
# 📊📧 MVP UNIVERSAL DE AUDITORIA + IA + EMAIL
# ==============================================
# Fluxo completo:
# 1. Leitura de JSONs de auditoria
# 2. Extração real de gap_de_conformidade (sem invenção)
# 3. IA gera 3 evidências por KPI
# 4. Envio de e-mail por colaborador
# 5. Pronto para Streamlit + Secrets
#
# ==============================================
...
#
# ⚠️ NUNCA versionar secrets.toml no Git
# ==============================================
        ...
        if file.endswith(".json"):
            ...
            with open(path, "r", encoding="utf-8") as f:
                try:
                    ...
                    st.warning(f"Erro no arquivo {file}: {e}")

    return dados

# =========================
# 2. IA: GERAR EVIDÊNCIAS
# =========================

def gerar_evidencias(kpi_nome, gaps):

    prompt = f"""
Você é um auditor corporativo racional.
...
Tarefa:
Gerar exatamente 3 evidências documentais reais e verificáveis
que eliminariam esses gaps.

Regras:
- NÃO inventar documentos inexistentes
- Ser genérico e aplicável a qualquer empresa
- Foco em auditoria real
...
"""
    ...
    response = client.chat.completions.create(
        model="gpt-4o",
        messages=[
            {"role": "system", "content": "Você é um auditor extremamente objetivo."},
            {"role": "user", "content": prompt}
        ],
        response_format={"type": "json_object"}
    )
...
def processar_auditoria(pasta_json: str):

    dados = carregar_jsons(pasta_json)

    resultados = []

    for item in dados:

        kpi = item.get("kpi_nome", "KPI_DESCONHECIDO")
        colaborador = item.get("colaborador", "SEM_NOME")
        gaps = item.get("gap_de_conformidade", [])

        gaps_txt = json.dumps(gaps, ensure_ascii=False)

        # IA
        evidencias = gerar_evidencias(kpi, gaps_txt)["evidencias"]
...
Status: AÇÃO CORRETIVA NECESSÁRIA
"""

        email_destino = f"{colaborador.lower()}@empresa.com"

        enviado = enviar_email(
            destino=email_destino,
            assunto=f"[AUDITORIA] KPI - {kpi}",
            corpo=corpo
        )
...
st.set_page_config(page_title="Auditoria IA + Email", layout="wide")

st.title("📊📧 MVP Auditoria Inteligente com IA + Email")

pasta = st.text_input("📁 Caminho da pasta com JSONs", "./data")

if st.button("🚀 Executar Auditoria"):

    resultados = processar_auditoria(pasta)

    st.subheader("📌 Resultado do Processo")
    st.json(resultados)
