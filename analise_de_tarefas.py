...
# ============================================================
# LOCALIZAÇÃO DINÂMICA DO DIRETÓRIO DE DADOS
# ============================================================

def buscar_diretorio_dados():
    # 1. Pega o diretório onde o script atual está rodando
    base_atual = os.path.dirname(os.path.abspath(__file__))
    
    # 2. Define uma lista de caminhos prováveis (Ordem de prioridade)
    # Isso cobre: local, servidor, e subpastas
    caminhos_tentativas = [
        os.path.join(base_atual, "dados"),
        os.path.join(base_atual, "formulario-colaborador", "dados"),
        os.path.join(os.getcwd(), "dados"),
        os.path.join(os.getcwd(), "formulario-colaborador", "dados")
    ]
    ...
    # 3. Fallback: Se não existir em lugar nenhum, cria na raiz do script
    diretorio_padrao = os.path.join(base_atual, "dados")
    os.makedirs(diretorio_padrao, exist_ok=True)
    return diretorio_padrao

# Define a variável global que o restante do seu código já usa
dados_dir = buscar_diretorio_dados()

# Mensagem de log para você verificar no console do Streamlit se ele achou o lugar certo
print(f"--- Sistema carregando dados de: {dados_dir} ---")
...
def carregar_todos_formularios():
    ...
    if os.path.exists(dados_dir):
        # Lista arquivos e garante que ignore pastas ou arquivos ocultos
        ...
        for nome_arquivo in arquivos:
            ...
            try:
                with open(caminho_completo, "r", encoding="utf-8") as f:
                    dados = json.load(f)
                    if isinstance(dados, dict):
                        # Adicionamos o nome do arquivo aos dados para conferência
                        dados['_origem'] = nome_arquivo 
