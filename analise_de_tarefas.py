...
# =========================================================
# 🧠 MOTOR COMPLETO — ANÁLISE INTELIGENTE (SEM CORTES)
# =========================================================
...
# 🔢 NORMALIZAÇÃO
# ---------------------------------------------------------
def tempo_em_minutos(item):
    try:
        h = str(item.get("Horas", "0")).replace(" h","").strip()
        m = str(item.get("Minutos", "0")).replace(" min","").strip()
...
def peso_frequencia(freq):
    mapa = {
        "DVD": 5, "D": 5, "S": 1, "Q": 0.5,
        "M": 0.25, "T": 0.08, "A": 0.02
    }
    return mapa.get(str(freq).strip().upper(), 1)

# ---------------------------------------------------------
# 📋 ATIVIDADES
# ---------------------------------------------------------
def score_atividades(atividades):
    ...
    for at in atividades:
        ...
        desc = str(at.get("Atividade","")).strip()
        tempo = tempo_em_minutos(at)
        freq = at.get("Frequência","")
        ...
        if tempo * peso_frequencia(freq) > 600:
            distorcao += 1

    p1 = preenchimento / total
    p2 = coerencia / total
    p3 = 1 - (distorcao / total)

    return round((p1*40 + p2*40 + p3*20), 2)

# ---------------------------------------------------------
# ⚠️ DIFICULDADES
# ---------------------------------------------------------
def score_dificuldades(dificuldades):

    if not dificuldades:
        return 0

    qualidade = 0
    total = len(dificuldades)

    for d in dificuldades:
        desc = str(d.get("Dificuldade","")).strip()
        setor = str(d.get("Setor Envolvido","")).strip()

        if desc and setor:
            qualidade += 1

    return round((qualidade/total)*100, 2)

# ---------------------------------------------------------
# 💡 SUGESTÕES
# ---------------------------------------------------------
def score_sugestoes(sugestoes):
    ...
    for s in sugestoes:
        desc = str(s.get("Sugestão","")).strip()
        impacto = str(s.get("Impacto Esperado","")).strip()
...
# =========================================================
# 🚀 EXECUÇÃO PELO SIDEBAR
# =========================================================
if st.session_state.get("pagina") == "analise":

    st.title("📊 Análise Inteligente dos Formulários")

    lista = carregar_todos_formularios()
    ...
    for form in lista:
        ...
        dificuldades = tabelas.get("dificuldades", [])
        sugestoes = tabelas.get("sugestoes", [])

        score_total_atividades = score_atividades(alta + normal + baixa)
        score_dif = score_dificuldades(dificuldades)
        score_sug = score_sugestoes(sugestoes)

        score_final = round(
            (score_total_atividades * 0.5) +
            (score_dif * 0.2) +
            (score_sug * 0.3),
            2
        )
        ...
        resultados.append({
            "Nome": form.get("colaborador", "N/A"),
            "Atividades": score_total_atividades,
            "Dificuldades": score_dif,
            "Sugestões": score_sug,
    ...
    dados = df[df["Nome"] == selecionado].iloc[0]

    col1, col2 = st.columns(2)

    col1.metric("Score Final", dados["Score Final"])
    col2.metric("Atividades", dados["Atividades"])

    col1.metric("Dificuldades", dados["Dificuldades"])
    col2.metric("Sugestões", dados["Sugestões"])
    ...
    score = dados["Score Final"]

    if score >= 80:
        st.success("🚀 Alta performance: excelente preenchimento, coerência e visão estratégica.")
    elif score >= 60:
        st.info("📈 Boa performance: há pequenas oportunidades de melhoria.")
    elif score >= 40:
        st.warning("⚠️ Performance mediana: inconsistências ou baixa profundidade.")
    else:
        st.error("❌ Baixa qualidade: dados incompletos ou incoerentes.")
...
# Se NÃO tiver ?page=formulario → NÃO mostra nada
if query_params.get("page") != "formulario":
    st.stop()

# Se chegou aqui, está na página correta
        ...
        if st.button("📥 Carregar Rascunho", key="btn_carregar_rascunho_v3"):
            if nome_f:
                ...
                if rascunho:
                    ...
                    cp = rascunho.get("colaborador", {})  # ✅ CORRETO AGORA
                    
                    # Mapeamento: "Chave_do_Widget": "Chave_dentro_do_JSON_colaborador"
                    campos_map = {
                        "f_nome_v2": "nome",
                        "f_cargo": "cargo",
